...
*Companion document to: ManSula Pay — Local Wallet Module PRD v2*
*Visual/UX inspiration reference: "Quantra — Smart Finance App" (Behance UI/UX case study)*
*Scope: Customer App · Merchant App · Admin (ManSula BOS back office)*
...
- An **admin** operator inside ManSula BOS doing reconciliation, settlement, and risk review.
...
| Admin (BOS) | Brand as accent only: neutral operational UI, gradient/brand color reserved for status highlights, charts, and primary actions — sits natively inside the existing ManSula BOS shell |

---

## 4. Assumptions baked into this PDD (please confirm)

1. **Platforms:** Customer and Merchant are native mobile apps (iOS + Android); Admin is a responsive web dashboard living inside the existing ManSula BOS shell — not a standalone app.
2. **Typography:** Gilroy (shown in your reference) is a commercial font. This document assumes Gilroy pending a licensing decision; **Poppins**, **Sora**, or **General Sans** are near-equivalent free alternatives if you'd rather not license it for now.
3. **Currency display:** MC is shown with a configurable symbol/label per PRD §3 (placeholder pending final naming — flagged again in §13).
4. **Admin visual scope:** this PDD designs the *Local Wallet module's screens* assuming they sit inside an existing BOS visual shell — not a full BOS redesign.
...
| Dimension | Customer App | Merchant App | Admin (BOS) |
|---|---|---|---|
...
## 9. Admin (Web Back Office within ManSula BOS)

### 9.1 Navigation (left sidebar, module within BOS)
...
5. Admin module integration into existing BOS shell
...
3. Does Admin need to visually match the existing ManSula BOS design system exactly, or can this module introduce its own accent language within that shell?
4. Is there an existing ManSula Pay logo/brand mark, or does one need to be designed as part of this pass?
