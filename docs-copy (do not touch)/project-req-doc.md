...
*Sub-module of the ManSula Pay Environment, used within ManSula BOS*
...
| Term | Meaning |
|---|---|
| Virtual currency (placeholder: **MC**) | The wallet's unit of value. Not "points" — a currency-like balance. Exchange rate to real currency (e.g. 1 MC = ₹1) is admin-configurable. |
| Wallet | A consumer's MC balance. |
| Funding / recharge | Any operation that adds MC to a wallet. |
| Sale / debit | Any operation that removes MC from a wallet in exchange for goods or services. |
...
- **Admin** (you / ManSula BOS back office)
- **Consumer** — may or may not carry a smartphone with the ManSula Pay consumer app
...
- **Live balance** — total credits received (MC collected from sales across all three payment modes) minus MC paid out via cash-in deposits/advance credits, i.e. the merchant's current unsettled balance, visible any time, not just at settlement.
...
- **Sales settlement (admin → merchant):** merchant collected MC from consumers → admin owes merchant real currency for it, minus commission.
- **Cash-in recovery (merchant → admin):** merchant collected real cash from consumers and credited MC directly → merchant now holds cash that belongs to the platform, and owes it back.

```
Net payout to merchant = (MC collected from sales × exchange rate) − (commission) − (cash collected via cash-in deposits)
```
...
| Consumer Wallet Liability | Liability | MC the platform owes consumers, redeemable at any merchant |
| Merchant Settlement Payable | Liability | Real currency the platform owes a merchant for MC they've collected from sales |
...
| Promotional Expense | Equity/P&L | Cost of MC granted with no real money behind it — promos, goodwill credits |
...
| Settlement payout, net positive | Merchant Settlement Payable; Platform Revenue (commission) | Platform Bank/Cash; Merchant Cash-in Receivable (cleared) |
